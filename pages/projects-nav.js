...
function loadProjectNavigation() {
    ...
    // Intentar insertar al final de #contenido-principal para que se alinee con las secciones
    const contentDiv = document.getElementById('contenido-principal');
    if (contentDiv) {
        contentDiv.appendChild(navContainer);
    } else {
        // Fallback: Intentar insertar antes del div de contacto
        const contactDiv = document.getElementById('contacto');
        if (contactDiv) {
            contactDiv.parentNode.insertBefore(navContainer, contactDiv);
        } else {
            console.error("✗ ERROR: No se encontró #contenido-principal ni #contacto para insertar navegación");
        }
    }
}
